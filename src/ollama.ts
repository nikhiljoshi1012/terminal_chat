...
export function prepareContext(
  ...
  const limited = max > 0 && messages.length > max ? messages.slice(messages.length - max) : messages
  return { system, messages: limited, dropped: messages.length - limited.length }
}
...
export async function chatStream(
  ...
  const payload: Message[] = system ? [{ role: "system", content: system, timestamp: 0 }, ...messages] : messages
  const res = await fetch(`${BASE}/api/chat`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ model, stream: true, think: true, messages: payload.map(toApi) }),
    signal,
  })
  if (!res.ok || !res.body) {
...
export async function generateTitle(prompt: string, model: string): Promise<string> {
  const instr = [
    {
      role: "system" as const,
      content: "Generate a concise title (3-5 words, no quotes, no punctuation) summarizing the user's request.",
    },
